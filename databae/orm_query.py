...
from databae.models import Banner, Cart, Category, Product, User, Finally_carts
...
async def orm_user_info(session:AsyncSession, chat_id:int)->User:
    query = select(User).where(User.user_id == chat_id)
    result =  await session.execute(query)
    return result.scalar()


async def orm_finally_add(session: AsyncSession,product_name, finally_price, quantity , cart_id):
    try:
        query = Finally_carts(product_name=product_name, finally_price=finally_price,quantity=quantity, cart_id=cart_id)
        session.add(query)
        await session.commit()
    except:
        session.rollback()
        query = update(Finally_carts).where(Finally_carts.product_name==product_name
                                                    ...
                                                    ).values(finally_price=finally_price,quantity=quantity)
        await session.execute(query)
        await session.commit()
        
async def  orm_update_users(
    session: AsyncSession,
    user_id:int,
    phone: str):
...
async def get_carts_order(session:AsyncSession, user_id):
    query = select(Cart).join(Product).where(Cart.user_id==user_id).filter(Cart.product)
    result = await session.execute(query)
    return result.scalars().all()

async def orms_get_user_carts(session: AsyncSession, user_id):
    query = select(Cart).filter(Cart.user_id == user_id).options(joinedload(Cart.product))
    result = await session.execute(query)
    return result.all()   


async def Final(session:AsyncSession, user_id):
    query = select(Cart.product)
    result = await session.execute(query)
    return result.fetchall()


async def  orm_delete_cart(session:AsyncSession, user_id):
